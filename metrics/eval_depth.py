...
def depth_evaluation_wrap(
    ...
    **kwargs
    ...
    if depth_alignment in align_methods:
        align_methods[depth_alignment] = True

    align_dict = {
        "align_with_lstsq": align_methods["lstsq"],
        "align_with_lad": align_methods["lad"],
        "align_with_lad2": align_methods["lad2"],
        "align_with_scale": align_methods["scale"],
    }


    return depth_evaluation(
        predicted_depth_original=predicted_depth_original,
        ground_truth_depth_original=ground_truth_depth_original,
...
def depth_evaluation(
    ...
    if max_depth is not None:
        ...
        mask = ground_truth_depth_original > 0
    mask = mask & custom_mask if custom_mask is not None else mask
    ...
    if disp_input:  # align the pred to gt in the disparity space
        ...
        ground_truth_depth = depth2disparity(ground_truth_depth)

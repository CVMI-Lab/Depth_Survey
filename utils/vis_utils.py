...
def save_rgb_depth(rgb, depth, save_path):
    """
    Save RGB and depth maps as a single image.
    :param rgb: RGB image, [H, W, 3]
    :param depth: Depth map, [H, W]
    :return: None
    """
    depth_colored = colorize_np(depth, cmap_name="Spectral_r", append_cbar=False)
    combined_image = np.concatenate((rgb, depth_colored), axis=1)
    combined_image = (combined_image * 255).astype(np.uint8)
    iio.imwrite(save_path, combined_image)
